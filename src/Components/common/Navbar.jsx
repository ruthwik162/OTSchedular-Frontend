...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-8 lg:px-16 xl:px-24 transition-all duration-500 z-50 ${isScrolled
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-4 lg:gap-8">
        {navLinks.map((link, i) => (
          ...
          >
            ...
            <div className={`h-0.5 w-0 group-hover:w-full transition-all duration-300 ${isScrolled ? "bg-gray-700" : "bg-indigo-500"
              }`} />
          </NavLink>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3 md:hidden">
        <motion.button
          ...
          className="p-2 bg-blue-900/50 rounded-md"
          aria-label="Toggle menu"
          aria-expanded={isMenuOpen}
          whileTap={{ scale: 0.9 }}
        >
          <svg
            className={`h-6 w-6 ${isScrolled ? "text-gray-800" : "text-black"}`}
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            viewBox="0 0 24 24"
              ...
              {navLinks.map((link, i) => (
                ...
                >
                  <NavLink
                    to={link.path}
                    className={({ isActive }) =>
                      `block px-4 py-3 rounded-lg ${isActive ? "bg-indigo-50 text-indigo-600" : "hover:bg-gray-50"}`
                    }
                    aria-current={location.pathname === link.path ? "page" : undefined}
                  >
            ...
            >
              {user && (
                <motion.button
                  ...
                  className="w-50 px-6 py-2 border border-indigo-600 text-indigo-600 hover:bg-indigo-50 rounded-full text-sm font-medium"
              ...
              {!user ? (
                <motion.button
                  onClick={() => setShowUserLogin(true)}
                  className="w-70 px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full text-sm font-medium shadow-sm"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  Login
                ...
                <motion.button
                  ...
                  className="w-50 px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full text-sm font-medium shadow-sm"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  Logout
