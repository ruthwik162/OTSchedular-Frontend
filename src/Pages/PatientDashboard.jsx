...
import { FiDownload, FiCalendar, FiPhone, FiMail, FiUser, FiDroplet, FiAlertTriangle, FiClock, FiScissors, FiLoader } from "react-icons/fi";
...
const PatientDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${URL}/ot/patient/${user.email}`);
      ...
      <div className="fixed inset-0 bg-white/90 backdrop-blur-sm z-50 flex items-center justify-center">
        <div className="text-center max-w-md px-6">
          ...
          <div className="">
            ...
            <p className="text-gray-600 flex flex-col items-center gap-4 justify-center">
              Preparing your OT details
              <span className="flex ml-1 space-x-2">
                {[0,1,2,3,4,5,6].map((i) => (
                  <span
                    key={i}
                    className="inline-block w-1.5 h-1.5 bg-indigo-400 rounded-full"
                    style={{
                      ...
                      <div className="space-y-3">
                        ...
                        {appt.assistantDoctor && (
                          <div>
                            ...
                            <p className="font-medium">{appt.assistantDoctor} ({appt.assistantDoctorEmail})</p>
                          </div>
                        )}
